...
from llff.poses.colmap_wrapper import run_colmap, undistort_image
...
def gen_poses(basedir, match_type, factors=None):
    ...
    if 'images_distort' not in os.listdir(basedir):
      print('NEED TO UNDISTORT IMAGES')
      undistort_image(basedir)
    print( 'Post-colmap')
    
    poses, pts3d, perm, hwf_cxcy = load_colmap_data(basedir)
    
    save_poses(basedir, poses, pts3d, perm, hwf_cxcy)
